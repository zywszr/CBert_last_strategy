...
class DDPG(object):
    def __init__(self, nb_states, nb_actions, args):
        ...
        # self.no_average = args.no_average
    ...
    def random_action(self, s_t):
        action_actor = to_numpy(self.actor(to_tensor(np.array(s_t).reshape(1, -1)))).squeeze(0)
        
        action = np.random.uniform(self.lbound, self.rbound, self.nb_actions)
        # self.a_t = action
        return action, action_actor
    ...
    def select_action(self, s_t, episode, decay_epsilon=True):
        ...
        action = sample_from_truncated_normal_distribution(lower=self.lbound, upper=self.rbound, mu=action_actor, sigma=delta)
